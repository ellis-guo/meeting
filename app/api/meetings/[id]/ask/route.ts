...
import { SOURCES_SEP, SSE_HEADERS, sseError, sseFrame as send } from "@/lib/sse";
...
export async function POST(
  ...
  if (!userId) return sseError("Unauthorized", 401);

  const rl = checkRateLimit(userId, "POST:/api/meetings/ask");
  if (!rl.allowed) {
    return sseError("请求过于频繁，请稍后再试（每分钟最多 20 次）", 429, {
  ...
  if (!apiKey) {
    return sseError("API key required. Please configure your DashScope API key in Settings.", 401);
  }
  ...
  if (!question?.trim() || question.length > 2000) {
    return sseError(
      !question?.trim() ? "question is required" : "question too long (max 2000 characters)",
      400,
    );
  }

  const raw = await prisma.meeting.findFirst({ where: { id: meetingId, user_id: userId } });
  if (!raw) return sseError("Meeting not found", 404);
